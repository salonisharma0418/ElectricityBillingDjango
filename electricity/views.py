...
def add_Bill(request):
    ...
    if request.method == "POST":
        ...
        current_bill_images=request.FILES.get('current_bill_images')
        print(current_bill_images)
        try:
        ...
        for rateobj in Unitrate.objects.order_by('initialunit'):
            if remaining_units > 0:
                ...
                d = {
                    'unit': rateobj.unitrate,
                    'rate': units_in_slab,
                    'rate_details': "Rs/U",
                    'amount': units_in_slab * rateobj.unitrate
                }
                l.append(d)
                finalamount += d['amount']
                remaining_units -= units_in_slab
                last_rateobj = rateobj
            else:
                break

        # Add load charge separately
        load_charge = calculate_load_charge(int(connection.connectionload))
        finalamount += load_charge
        ...
        try:
            ...
            if check_data:
                error = "yes"  # Update error code
            else:
                total_bill_amt = sum(d['amount'] for d in l) + load_charge
                bill_data = Bill.objects.create(
                    connection=connection,
                    billformonth=current_date,
                    currentreading=creading,
                    previousreading=preading,
                    totalunit=Decimal(total_units),
                    chargeperunit=last_rateobj.unitrate if last_rateobj else 0,
                    finalamount=Decimal(finalamount),
                    duedate=dd,
                    status='Not Paid',
                    total_bill_amt=total_bill_amt,
                    current_bill_images=current_bill_images
                )
                error = "no"
                if bill_data:
                    ...
                    Bill_load.objects.create(
                        ...
                        load_rate=load_charge,
                        load_rate_details='Rs/KW',
                        load_amount=float(connection.connectionload) * load_charge,
                    ...
                    Bill_duty.objects.create(
                        ...
                        duty_amount=(total_units * 11) / 100,
    ...
    return render(request, 'add_Bill.html', {'error': error, 'connection1': connection1})
